...
def main():
    ...
    for i in range(dataProcessingBatchNum): 
        for player_mode in twoPlayer_mode:  
            dc.addComment("%s -- %s\n"%(i, player_mode))
            re = handleOne(model,autoencoder,startIndexOfImage+i,manipulations[0],dc,player_mode)
...
def handleOne(model,autoencoder,startIndexOfImage,manipulationType,dc,player_mode):
    ...
    print("two player game with the second player being %s ... "%player_mode)
    ...
    if re == True:     
        ...
        plt.savefig(path0)
        path1="%s/%s_%s_%s_modified_into_%s_heatmap.png"%(directory_pic_string,startIndexOfImage,manipulationType, origClassStr,newClassStr)
    ...
    path = "%s/%s_%s.mat"%(directory_pic_string,startIndexOfImage,manipulationType)
    st.re_training.saveToMat(path)
    st.destructor()
