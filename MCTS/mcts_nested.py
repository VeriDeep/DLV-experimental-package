...
class mcts:
    ...
    def initialiseActions(self): 
        ...
        self.pocketTree = decisionTree(self.model, self.actions,self.activations, "pocket")
    ...
    def sampleNext(self,spansPath,numSpansPath,depth,availableActionIDs,usedActionIDs,accDims,d): 
        ...
        if newClass != self.originalClass and newConfident > effectiveConfidenceWhenChanging:
            ...
            self.pocketActivations = activations1
            sampleValues = []
            for i in range(MCTS_multi_samples): 
                (childTerminated, val) = self.sampleNextSndRound(spansPath,numSpansPath,depth,availableActionIDs,usedActionIDs,accDims,d)
                sampleValues.append(val)
    ...
    def sampleNextSndRound(self,spansPath,numSpansPath,depth,availableActionIDs,usedActionIDs,accDims,d): 
        activations1 = applyManipulation(self.activations,spansPath,numSpansPath)
        (newClass,newConfident) = self.predictWithActivations(activations1)
        (distMethod,distVal) = controlledSearch
        ...
        if distMethod == "euclidean": 
            ...
            dist = self.activations.size - diffPercent(activations1,self.pocketActivations) * self.pocketActivations.size
            termValue = 0.0
        ...
        if newClass == self.originalClass:
            ...
            return self.sampleNextSndRound(newSpanPath,newNumSpanPath,depth+1,availableActionIDs,usedActionIDs,accDims,d)
    ...
    def showPocketTree(self):
        self.pocketTree.show()
        self.pocketTree.outputTree()
